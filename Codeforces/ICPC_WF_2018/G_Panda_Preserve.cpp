...
struct VEdge {
    pt a, b; // Endpoints of the Voronoi edge
    int s1, s2; // The two sites this edge bisects
};
...
struct fortune {
    ...
    vector<pt> vor_verts;
    vector<VEdge> vor_edges;
    // Map to track the "active" Voronoi vertex for a pair of sites
    map<pair<int, int>, pt> last_vert;
    fortune(vector<pt> p) {
        n = sz(p);
        v.resize(n);
        rep(i, 0, n) v[i] = {p[i], i};
    ...
    // Records a Voronoi edge segment between two circumcenters
    void add_vor_edge(int i, int j, pt new_v) {
        if (i == -1 || j == -1) return;
        if (i > j) swap(i, j);
        if (last_vert.count({i, j})) {
            vor_edges.push_back({last_vert[{i, j}], new_v, i, j});
        }
        last_vert[{i, j}] = new_v;
    }
    ...
    void remove(beach::iterator it) {
        ...
        pt c = circumcenter(it->p, it->q, a->p);
        vor_verts.push_back(c);
        
        // When an arc is removed, three Voronoi edges meet at circumcenter 'c'
        add_vor_edge(a->i, it->i, c);
        add_vor_edge(it->i, b->i, c);
        add_vor_edge(a->i, b->i, c);
