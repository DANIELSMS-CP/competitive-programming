...
void solve()
{
    ...
    vector<vi> tree_adj(n);
    vector<int> edge_weight(n, 0);
    ...
    HLD<true> hld(tree_adj); 

    for (Edge e:result) {
        int child=((hld.depth[e.u]>hld.depth[e.v])?e.u:e.v);
        hld.modifyNode(child,e.weight);
    }
    vector<int> ans(m);
    for(int i=0;i<m;i++)
    {
        ans[edges[i].idx]=cost-hld.queryPath(edges[i].u,edges[i].v)+edges[i].weight;
    }
    for(int i=0;i<m;i++)
    {
        cout << ans[i] << '\n';
